...
private:
    int sfd;
    const char* saddr = "10.9.0.6";  // Source IP
    const char* daddr = "10.9.0.5";  // Destination IP
    int sport = 9090;                // Source Port
    int dport = 1023;                // Destination Port (as in the image)
    long long seq_num = 3958269143;        // SYN packet sequence number (from the image)
    long long ack_num;                     // To be set dynamically
...
public:
    ...
    void sendPacket() {
        ...
        tcp->seq = htonl(seq_num);     // Sequence Number (from the image)
        tcp->ack_seq = htonl(ack_num); // Acknowledgment Number (for SYN+ACK)
	cout<<ack_num<<" "<<ntohl(tcp->ack_seq)<<endl;
        tcp->doff = 5;                
        ...
        if (sz < 0) {
            ...
            std::cout << "Successfully sent SYN+ACK packet\n";
        }
    }
};
...
int main() {
    long long ackNum;
    cout << "Enter the acknowledgment number: ";
    cin >> ackNum;
